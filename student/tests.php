<?php
require_once '../includes/config.php';
require_once '../includes/db.php';
requireStudent();

$branch_id = $_SESSION['branch_id'];
$student_id = $_SESSION['user_id'];
$subject_id = (int)($_GET['subject_id'] ?? 0);

$subjects = $db->fetchAll("SELECT * FROM subjects WHERE branch_id = ? ORDER BY subject_name", [$branch_id]);

$conditions = "WHERE t.branch_id = ?";
$params = [$branch_id];

if ($subject_id > 0) {
    $subject = $db->fetch("SELECT id FROM subjects WHERE id = ? AND branch_id = ?", [$subject_id, $branch_id]);
    if ($subject) {
        $conditions .= " AND t.subject_id = ?";
        $params[] = $subject_id;
    }
}

$testQuery = "SELECT t.*, s.subject_name,
    (SELECT COUNT(*) FROM test_questions WHERE test_id = t.id) AS question_count,
    (SELECT id FROM test_attempts WHERE test_id = t.id AND student_id = ?) AS attempted
    FROM monthly_tests t
    JOIN subjects s ON t.subject_id = s.id
    $conditions
    ORDER BY t.created_at DESC";

$tests = $db->fetchAll($testQuery, array_merge([$student_id], $params));
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Monthly Tests - <?= SITE_NAME ?></title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap" rel="stylesheet">
    <link href="<?= getUrl('assets/css/style.css') ?>" rel="stylesheet">
</head>
<body>
    <?php include 'navbar.php'; ?>
    <div class="container-fluid py-4">
        <div class="container">
            <h4 class="fw-bold mb-4"><i class="fas fa-calendar-check me-2 text-primary"></i>Monthly Tests</h4>
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <form method="GET" class="row g-2 align-items-end">
                        <div class="col-md-4">
                            <label class="form-label fw-medium small">Subject</label>
                            <select name="subject_id" class="form-select" onchange="this.form.submit()">
                                <option value="">All Subjects</option>
                                <?php foreach ($subjects as $subject): ?>
                                <option value="<?= $subject['id'] ?>" <?= $subject_id === (int)$subject['id'] ? 'selected' : '' ?>>
                                    <?= sanitize($subject['subject_name']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-2">
                            <a href="tests.php" class="btn btn-outline-secondary w-100 rounded-pill">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
            <div class="row g-3">
                <?php foreach ($tests as $t): ?>
                <div class="col-md-6 col-lg-4">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body">
                            <div class="d-flex justify-content-between mb-2">
                                <h6 class="fw-bold mb-0"><?= sanitize($t['title']) ?></h6>
                                <?php if ($t['attempted']): ?>
                                <span class="badge bg-success-soft text-success rounded-pill"><i class="fas fa-check me-1"></i>Done</span>
                                <?php endif; ?>
                            </div>
                            <p class="small text-muted mb-2"><i class="fas fa-book me-1"></i><?= sanitize($t['subject_name']) ?></p>
                            <p class="small text-muted mb-3">
                                <i class="fas fa-list-ol me-1"></i><?= $t['question_count'] ?> Questions
                                <?php if ($t['description']): ?> | <?= sanitize($t['description']) ?><?php endif; ?>
                            </p>
                            <?php if ($t['attempted']): ?>
                                <?php $attempt = $db->fetch("SELECT * FROM test_attempts WHERE test_id = ? AND student_id = ?", [$t['id'], $student_id]); ?>
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="fw-bold text-<?= $attempt['score'] >= $attempt['total_questions']/2 ? 'success' : 'danger' ?>">
                                        Score: <?= $attempt['score'] ?>/<?= $attempt['total_questions'] ?>
                                    </span>
                                    <a href="test_result.php?id=<?= $t['id'] ?>" class="btn btn-sm btn-outline-primary rounded-pill">View Result</a>
                                </div>
                            <?php else: ?>
                            <a href="take_test.php?id=<?= $t['id'] ?>" class="btn btn-warning btn-sm rounded-pill w-100">
                                <i class="fas fa-play me-1"></i>Start Test
                            </a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
                <?php if (empty($tests)): ?>
                <div class="col-12 text-center py-5">
                    <i class="fas fa-calendar-check fs-1 text-muted mb-3"></i>
                    <p class="text-muted">No monthly tests available for your branch yet.</p>
                </div>
                <?php endif; ?>
            </div>
        </div>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
