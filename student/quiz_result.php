...
$attempt = $db->fetch("SELECT * FROM quiz_attempts WHERE quiz_id = ? AND student_id = ?", [$quiz_id, $student_id]);
if (!$attempt) { flashMessage('danger', 'You have not attempted this quiz.'); redirect('student/quizzes.php'); }
...
<head>
    ...
    <title>Quiz Result - <?= SITE_NAME ?></title>
        ...
        <div class="container">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-body text-center py-5">
                    ...
                    <p class="text-muted"><?= sanitize($quiz['title']) ?></p>
                    <div class="d-flex justify-content-center gap-4 my-4">
                        <div>
                            <div class="fs-1 fw-bold text-success"><?= $attempt['score'] ?></div>
                            <small class="text-muted">Correct</small>
                        </div>
                        <div class="border-start ps-4">
                            <div class="fs-1 fw-bold text-danger"><?= $attempt['total_questions'] - $attempt['score'] ?></div>
                            <small class="text-muted">Wrong</small>
                        </div>
                        <div class="border-start ps-4">
                            <div class="fs-1 fw-bold text-primary"><?= $attempt['total_questions'] ?></div>
                            <small class="text-muted">Total</small>
                        </div>
                        <div class="border-start ps-4">
                            <div class="fs-1 fw-bold <?= $passed ? 'text-success' : 'text-danger' ?>"><?= $percentage ?>%</div>
                            <small class="text-muted">Percentage</small>
                        </div>
                    </div>
                    <a href="quizzes.php" class="btn btn-primary rounded-pill px-4"><i class="fas fa-arrow-left me-2"></i>Back to Quizzes</a>
                </div>
            </div>
                        ...
                        <div class="row g-1">
                            ...
                            <div class="col-sm-6">
                                <span class="badge bg-<?= strtoupper($q['correct_answer']) === $opt ? 'success' : 'light' ?> text-<?= strtoupper($q['correct_answer']) === $opt ? 'white' : 'dark' ?> p-2 w-100 text-start">
                                    ...
                                    <?php if (strtoupper($q['correct_answer']) === $opt): ?>
